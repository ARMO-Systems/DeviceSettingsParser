...
namespace ArmoSystems.ArmoGet.DeviceSettingsParser
{
    internal sealed class DeviceSettingsOperator
    {
        ...
        private const string SdkTypeEnumName = "DeviceTypeInfo1XPO.SDKEType";
        ...
        private readonly List< Terminal > allTerminals = new List< Terminal >();
        ...
        public void CreateAutogeneratedDevicesInfo( string autogeneratedDir )
        {
            using ( var file = new StreamWriter( $@"{autogeneratedDir}\IDeviceInfo.cs" ) )
            ...
            foreach ( var terminal in allTerminals )
            {
                ...
                using ( var file = new StreamWriter( $@"{autogeneratedDir}\{terminalClassName}.cs" ) )
        ...
        private string CreateDeviceInfo( Terminal terminal, string terminalClassName )
        {
            ...
            fileContent.Append( @"using ArmoSystems.Timex.Common.ExternalSystems;
using ArmoSystems.Timex.SDK.ExternalSystem;
using ArmoSystems.Timex.Shared.XPO.Source.Devices;

namespace ArmoSystems.Timex.PullZKPlugin
{" );
            fileContent.Append( $"    public sealed class {terminalClassName}: IDeviceInfo" );
            fileContent.Append( @"
    {" );
            foreach ( var prop in propInfo.OrderBy( prop => prop.Name ) )
            {
                ...
                fileContent.Append(
                    $"        public {( prop.Name == "RiDefault" ? IdentificationModeEnumName : prop.Name == "FType" ? SdkTypeEnumName : ConvertToShortType( prop.PropertyType.ToString() ) )} {prop.Name} => {GetValueFromProperty( prop, terminal )};" );
            }
...
namespace ArmoSystems.Timex.Common.ExternalSystems
{
    ...
    {" );
            ...
            foreach ( var prop in propInfo.OrderBy( prop => prop.Name ) )
            {
                ...
                fileContent.Append( $"        {( prop.Name == "RiDefault" ? IdentificationModeEnumName : prop.Name == "FType" ? SdkTypeEnumName : ConvertToShortType( prop.PropertyType.ToString() ) )} {fieldName} {{ get; }}" );
            }
            fileContent.Append( Environment.NewLine + "    }" + Environment.NewLine + '}' );
            return fileContent.ToString();
        }
...
namespace ArmoSystems.Timex.Common.Database" );
        ...
        private string GetSetupFuncs()
        {
            ...
            foreach ( var terminal in terminalList )
            {
                ...
                for ( var i = 0; i < propInfo.Count(); i++ )
                {
                    ...
                    sb.Append( GetValueFromProperty( prop, terminal ) );
        ...
        private static string GetValueFromProperty( PropertyInfo prop, Terminal terminal )
        {
